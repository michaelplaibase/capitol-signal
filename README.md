...
Interactions: search or tap a politician's name to filter the feed; an
amount-band filter (1k-15k up to 25M+) and quick-chips for notable committee
members (Pelosi, Trump, Tuberville, ...) that filter the feed and run the
simulator; every trade shows the market close on the trade date plus the current
price (api/series.js, loaded once per ticker); and a Simulator (api/simulate.js)
that values a politician's disclosed buys since a chosen date, using their full
Kadoa filer history and Yahoo closing prices. All price and simulator figures are
estimates (midpoint of the amount range, close on the trade date, buy-and-hold),
not the disclosed execution price, which is not published.
